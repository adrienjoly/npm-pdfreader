...
function makeAccumulator(nbRows, rowsToSkip){
  ...
  rule.whenDone(function(){
    ...
    this.output.rows = buildRowList(items, classifyRow).slice(rowsToSkip || 0);
    // classify row items into columns
    var classifyColumn = makeColumnClassifier(this.output.rows[0]);
